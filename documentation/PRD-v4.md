# Makamakam — Product Requirements v4
...
*Supersedes v3. Three things changed: one rule is now applied consistently, one
threshold became derived rather than guessed, and a field visit produced two
findings that break an assumption in the data model.*

*In short: **only the check-in requires presence**; the wall takes **one kind of
post** rather than two; **block, row and plot are optional labels** rather than
structure; the visual handoff is **derived from angular error**
rather than fixed at 8 m; and the graves are managed by a **kelompok**, not by
individual families — which is both the answer to §10's enforcement problem and a
hole in §8.*
...
**What changed in v4.** v3 kept prayer text behind presence while stating this
rule, which contradicted it: a prayer is words, and the rule's own reasoning is
that a doa recited alone in a room is valid. The lock is gone.

Exactly one thing in this product now requires standing at the grave: the
**check-in**, and the kamboja attached to it. Everything else — the name, the
parentage, the dates, the photographs, the location, the plan, the prayer text,
the wall and the profile — is reachable from anywhere, subject only to
`wallVisibility`.
...
Two corrections from the field, both from standing at the gate.

**The graves are not laid out in rows.** Markers sit where the ground allowed,
around a tree, at angles. Any design that assumes a grid describes a filing system
rather than a place. See §8.

**Some stones carry no name and no dates.** This is the hardest available proof of
the premise. A visitor cannot find those graves by walking and reading, however
long they are willing to look, and neither can the family of the person two plots
over. Wayfinding here is not a convenience the caretaker could otherwise provide —
for these graves it is the only route there is. It also breaks the survey method
in §14: a name cannot be read off a blank stone, so it has to come from a person
who knows.

**The graves are managed collectively.** A *kelompok* manages blocks of graves,
rather than each family managing its own. This changes who the product talks to
(§5), where the records come from (§14), and who can vouch for a family's claim
to a grave (§10).
...
**Also a user — the pengurus.** The caretaker or committee keeps the records
current through the admin site. v2 put cemetery operators out of scope; that
remains true of *plot inventory and burial rights*, but somebody has to be able
to correct a name without a developer.

**Newly — the kelompok.** A group that manages a block of graves, sitting between
the pengurus and the family: closer to the graves than the committee, wider than
one household. They are likely to hold the written list (§14), and they are the
people who can say which family belongs to which grave (§10). The product does not
address them yet, and should.
...
**Errors compound, and the arrow does not degrade gently.** What matters on
approach is not the distance error but the *direction* error it produces: with a
combined error `e` at distance `d`, the arrow can be wrong by `atan(e / d)` — about
6° at 30 m, and 45° at 3 m for a 3 m error. It falls off a cliff.

Three things make the real error larger than the figure on screen:

- the **surveyed coordinate carries its own error** on top of the visitor's, so
  two independent 3 m errors combine to about 4.2 m;
- **`horizontalAccuracy` is a 68% radius**, so roughly one reading in three is
  worse than reported;
- **the quoted figures are open-sky**, and this site has kamboja throughout, a
  boundary wall and stones — exactly what degrades GNSS.

So the handoff is derived rather than chosen: **hand over where the arrow's
direction error would exceed 20°.** That puts it near 6 m rather than the 8 m
guessed in v3, clamped to 5–15 m so good GPS keeps the arrow alive longer and poor
GPS gives up sooner. It is why positions are surveyed as offsets (§8).
...
| `section` / `row` / `plot` | **Optional, and only a label.** The layout comes from surveyed offsets, and at this site the stones are not in rows at all — so a grave may have no ledger number, and most will not. Where one exists it appears on the grave's own screen and, alongside the years, in a list of graves inside a cemetery. It appears nowhere on the walk: a reference number does not help anyone identify the stone in front of them. |
...
| *(name)* | **May be unknown.** Some stones carry nothing. The model already allows a grave with no death date; it must also tolerate a grave whose name came from the kelompok's list rather than from the stone, and say which. |
| `stewardName` | The family member who holds the record. **Known gap:** stewardship at this site is collective — a kelompok, not a person — and a single name cannot express that. Options: a `stewardGroup` alongside the individual; a list of steward identities; or a group record that families are linked to. Not settled here, because it needs another conversation with the group before it is designed. |
...
**No presence required** — see §3. The flower and the visit log moved to the wall.
...
remove any of them, and sets whether the wall is open, family-only, or closed.
Name, photographs, dates and location remain visible regardless — a grave must
stay findable even when its family wants no wall.

v3 had a check-in and a story as separate posts. They are the same impulse with
different fields filled in, and splitting them reintroduced exactly the
two-surfaces problem that got private notes removed: two composers, two sets of
rules, two explanations. One post type, one composer, and where the phone is
decides what the post becomes.
...
**What the steward governs, and what they do not:**
...
**Moderation is post-moderation, not pre-moderation.** A post is on the wall the
moment it is written and leaves only if the steward takes it down. Three reasons:
CloudKit cannot let a steward edit a stranger's record, so approval could never
have been a field on the post; `WallDecision` already works this way, as a
separate record made by the steward with the latest one winning; and a friend who
writes at 2am and hears nothing for a week will not write again. The cost —
something unwanted is briefly visible — is smaller than the cost of a queue
nobody empties.

**The database cannot enforce stewardship.** Anyone with the app could write a
claim. The answer is now in the field rather than in the schema: the **kelompok**
already knows which family belongs to which grave, so stewardship should be
vouched for by them and granted from the admin with the server key, which only the
operator holds. Until then, first-claim-wins is acceptable for one cemetery and
not beyond it.
...
- **The handoff distance is computed, not configured**: `combined / tan(20°)`,
  clamped to 5–15 m, where `combined` is the visitor's `horizontalAccuracy` and
  the survey's own accuracy added in quadrature. The near phase is kept
  proportional to it, so the two cannot cross on a bad day.
- **The arrow shrinks as it closes**, inverting what an AirTag does. A UWB radio
  at both ends earns growing confidence; a phone does the opposite, because the
  error stays fixed while the distance shrinks. This is deliberate and commented
  as such in the source so nobody "fixes" it.
- **The card where a person is chosen carries three fields**: name, parentage,
  year of death. That is what answers *is this them*. Faith, photographs, wall and
  profile belong after arrival.
...
4. **Ask the kelompok for their list first.** A group that collects dues keeps a
   record, and it is both the fastest cold start and the *only* possible source
   for the graves whose stones carry no name. The survey then confirms what the
   list says rather than starting from nothing.
...
- **Was opening Tend right?** v4 removed the last lock on prayer text. Ask the
  same families in the same conversation as the wall question — they are the same
  question about what presence is for.
- **What does the kelompok actually do?** Maintenance, dues, burial arrangements —
  and does anyone know? Do they keep a **written list**, and in what form?
- **Can anyone name the uninscribed graves?** If the kelompok can, that is the
  cold-start source and the only route those graves have. If nobody can, the
  product should say so rather than leave a blank.
...
- Correct tuning for the haptic curve.
